...
def predict(data: FlowData):
    ...
    features = {
        ...
        "Fwd Packets Length Total": data.Fwd_Packets_Length_Total,
        "Bwd Packets Length Total": data.Bwd_Packets_Length_Total,
        ...
        "FIN Flag Count": data.FIN_Flag_Count
    }
...
def get_stats():
    ...
    for row in predictions:
        ...
        try:
            ...
            probabilities.append(
                float(row.get("attack_probability", 0))
            )
    ...
    allowed = sum(
        1
        for row in predictions
        if row.get("firewall_action") == "ALLOW"
    )

    monitored = sum(
        1
        for row in predictions
        if row.get("firewall_action") == "MONITOR"
    )

    blocked = sum(
        1
        for row in predictions
        if row.get("firewall_action") == "BLOCK"
    )
    ...
    return {
        ...
        "attack_predictions": attack,

        "average_attack_probability": round(
            average_probability,
            4
        ),
